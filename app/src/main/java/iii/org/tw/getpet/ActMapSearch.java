...
public class ActMapSearch extends AppCompatActivity implements OnMapReadyCallback {
    ...
    CountDownLatch iv_CountDownLatch;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        iv_CountDownLatch = new CountDownLatch(1);
    ...
    public void onMapReady(final GoogleMap googleMap) {
        ...
        try {
            ...
            Log.d("GPS定位是否開啟",locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)+"");
            Log.d("NETWORK定位是否開啟",locationManager.isProviderEnabled(LocationManager.NETWORK_PROVIDER)+"");
            if(locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)||
                    locationManager.isProviderEnabled(LocationManager.NETWORK_PROVIDER))
            {
                Location gps=locationManager.getLastKnownLocation("network");
                if(gps!=null){
                    StringBuffer message=new StringBuffer();
                    message.append("位置：緯度:");
                    message.append(Double.toString(gps.getLongitude()));
                    message.append(",經度：");
                    message.append(Double.toString(gps.getLatitude()));
                    Log.d("取到的經緯度",message.toString());
                    longitude = gps.getLongitude();
                    latitude = gps.getLatitude();

                }
                else
                    Toast.makeText(this,"定位不到目前位置",Toast.LENGTH_LONG).show();
            }else
                Toast.makeText(this,"沒有開啟定位功能",Toast.LENGTH_LONG).show();
...
//            googleMap.animateCamera(CameraUpdateFactory.newCameraPosition(cameraPosition));
            ...
            Toast.makeText(this,"請確認網路訊號是否良好",Toast.LENGTH_LONG).show();
        }
    }
